...
vi.mock("./db", () => ({
  ...
  setParallelJourneyStage: vi.fn().mockImplementation(async (input: any) => ({ id: 1, ...input, status: "pendente" })),
  getChurchUsersByChurch: vi.fn().mockResolvedValue([]),
  getChurchUserByEmail: vi.fn().mockResolvedValue(null),
  ...
  getPublicAnnouncementsByChurch: vi.fn().mockResolvedValue([]),
...
describe("Fluxo completo de discipulado", () => {
  ...
  describe("Discipulador principal", () => {
    it("define o discipulador somente pela mutation explícita e tenant-aware", async () => {
      const caller = appRouter.createCaller(createMemberContext());

      await caller.people.setPrimaryDiscipler({
        churchId: CHURCH_ID,
        personId: 1,
      ...
      expect(setPrimaryDiscipler).toHaveBeenCalledWith({
        churchId: CHURCH_ID,
        personId: 1,
        disciplerPersonId: 10,
        notes: "Responsabilidade definida pela supervisão.",
    ...
    it("bloqueia a definição por membro sem papel de liderança", async () => {
      ...
      await expect(caller.people.setPrimaryDiscipler({
        churchId: CHURCH_ID,
        personId: 1,
        disciplerPersonId: 10,
      })).rejects.toThrow("Somente Pastores ou Supervisores");
      expect(setPrimaryDiscipler).not.toHaveBeenCalled();
    });

    it("faz care.assign com papel discipulador delegar para a fonte de verdade", async () => {
      const caller = appRouter.createCaller(createMemberContext());
      ...
      await caller.care.assign({
        ...
        role: "discipulador",
        notes: "Atribuição explícita.",
      });

      expect(setPrimaryDiscipler).toHaveBeenCalledWith(expect.objectContaining({
        churchId: CHURCH_ID,
        personId: 1,
        disciplerPersonId: 10,
      }));
  ...
  describe("Etapa 1 — Cadastro de nova alma", () => {
    it("cria uma nova alma com dados válidos", async () => {
      ...
      expect(result.careAssignment).toBeNull();
      expect(result.needsDiscipler).toBe(true);
      expect(setCurrentCareAssignment).not.toHaveBeenCalled();
    });
  ...
  describe("Etapa 2 — Consolidação", () => {
    ...
    it("permite ao Pastor Presidente ativar uma frente paralela sem trocar a etapa principal", async () => {
      const caller = appRouter.createCaller(createMemberContext());

      await expect(caller.people.setParallelJourneyStage({
        churchId: CHURCH_ID,
        id: 10,
        stage: "fundamentos",
        isCurrent: true,
      })).resolves.toMatchObject({ stage: "fundamentos", isCurrent: true });

      expect(setParallelJourneyStage).toHaveBeenCalledWith({
        churchId: CHURCH_ID,
        personId: 10,
        stage: "fundamentos",
        isCurrent: true,
    ...
    it("bloqueia frente paralela para conta sem papel pastoral", async () => {
      (getChurchMemberByUserId as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
        id: 2,
        userId: 10,
        churchId: CHURCH_ID,
      ...
      await expect(caller.people.setParallelJourneyStage({
        ...
        stage: "consolidacao",
        isCurrent: true,
      })).rejects.toThrow("Somente o Pastor Presidente ou Pastor Local");
      expect(setParallelJourneyStage).not.toHaveBeenCalled();
    });
  ...
  describe("Etapa 2.6 — Serviço em Ministério", () => {
    it("permite ao Pastor criar o Ministério de Consolidação com sua estrutura", async () => {
      (createMinistry as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ id: 9, churchId: CHURCH_ID, name: "Consolidação e Visitas", type: "consolidacao", leaderId: 10, active: true });
      const caller = appRouter.createCaller(createMemberContext());

      const ministry = await caller.ministries.create({ churchId: CHURCH_ID, name: "Consolidação e Visitas", type: "consolidacao", leaderId: 10, participantIds: [11] });
    ...
    it("libera a aba e a função de Consolidador para usuário vinculado a participante ativo", async () => {
      (getActiveChurchUserById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ id: 2, churchId: CHURCH_ID, personId: 10, role: "membro", active: true });
      (isActiveConsolidationMinistryMember as ReturnType<typeof vi.fn>).mockResolvedValueOnce(true);
      const caller = appRouter.createCaller(createMemberContext(-2));

      const access = await caller.churchAuth.accessSummary({ churchId: CHURCH_ID });

      expect(access.isConsolidator).toBe(true);
      expect(access.isPastoralWorker).toBe(true);
      expect(access.roles).toContain("consolidador");
      expect(isActiveConsolidationMinistryMember).toHaveBeenCalledWith(10, CHURCH_ID);
    });
  ...
  describe("Consolidação — encaminhamento de resgate", () => {
    ...
    it("permite que a liderança pastoral aceite a indicação antes do cuidado", async () => {
      ...
      (getActiveChurchUserById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ id: 2, churchId: CHURCH_ID, personId: 10, role: "pastor_presidente", active: true });
      vi.mocked(getConsolidationReferralById).mockResolvedValueOnce({
        ...
        preferredConsolidatorId: 10,
        status: "pendente",
      } as any);
      const caller = appRouter.createCaller(createMemberContext(-2));

      await caller.consolidation.approveReferral({ churchId: CHURCH_ID, id: 51, notes: "Aceito pela liderança pastoral." });

      expect(approveConsolidationCase).toHaveBeenCalledWith(expect.objectContaining({
        churchId: CHURCH_ID,
        referralId: 51,
        approvedByPersonId: 10,
        notes: "Aceito pela liderança pastoral.",
      }));
    });
  ...
  describe("Eventos — criação e relatório de presença", () => {
    ...
    it("permite ao Pastor confirmar pagamento de inscrição pública sem Pessoa vinculada", async () => {
      ...
      const caller = appRouter.createCaller(createMemberContext(-21));

      const result = await caller.events.setPaymentStatus({
        churchId: CHURCH_ID,
        eventId: 31,
        registrationId: 501,
        paymentStatus: "pago",
      ...
      expect(result).toEqual({ success: true });
      expect(updateEventRegistrationPayment).toHaveBeenCalledWith({
        churchId: CHURCH_ID,
        eventId: 31,
        registrationId: 501,
        paymentStatus: "pago",
        confirmedByChurchUserId: 21,
      });
    });

    it("mantém o isolamento do tenant ao tentar alterar uma inscrição externa", async () => {
      (getActiveChurchUserById as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
        id: 21,
        churchId: CHURCH_ID,
        personId: null,
      ...
      await expect(caller.events.setPaymentStatus({
        churchId: 999,
        eventId: 31,
        registrationId: 501,
        paymentStatus: "pago",
      })).rejects.toThrow("Acesso negado a esta igreja");
      expect(updateEventRegistrationPayment).not.toHaveBeenCalled();
    });
  });
