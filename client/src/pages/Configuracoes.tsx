...
function svgTextBlockAligned(value: string, x: number, y: number, maxCharacters: number, maxLines: number, fontSize: number, fill: string, weight = 400) {
  return wrapSvgText(value, maxCharacters, maxLines).map((line, index) => `<text x="${x}" y="${y + index * (fontSize + 10)}" text-anchor="start" font-family="Arial, sans-serif" font-size="${fontSize}px" font-weight="${weight}" fill="${fill}">${escapeSvgText(line)}</text>`).join("");
}
...
function buildQrLandscapeSvg({ qrSource, churchName, title, message, campaign, primaryColor, secondaryColor, logoDataUrl }: { qrSource: string; churchName: string; title: string; message: string; campaign: string; primaryColor: string; secondaryColor: string; logoDataUrl?: string | null }) {
  ...
  const logo = logoDataUrl ? `<image href="${logoDataUrl}" x="150" y="100" width="150" height="150" preserveAspectRatio="xMidYMid meet" />` : "";
  const campaignBlock = safeCampaign ? svgTextBlockAligned(safeCampaign, 150, 690, 38, 1, 28, primaryColor, 700) : "";
  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="1600" height="900" viewBox="0 0 1600 900"><rect width="1600" height="900" rx="48" fill="#ffffff"/><rect width="1600" height="16" fill="${primaryColor}"/><rect x="42" y="42" width="1516" height="816" rx="34" fill="#ffffff" stroke="${secondaryColor}" stroke-width="5"/><rect x="900" y="80" width="2" height="740" fill="${secondaryColor}" opacity="0.45"/><circle cx="1230" cy="430" r="310" fill="${primaryColor}" opacity="0.06"/>${logo}${svgTextBlockAligned(safeChurchName, 150, 300, 34, 2, 42, primaryColor, 700)}${svgTextBlockAligned(safeTitle, 150, 390, 36, 2, 34, "#172033", 700)}${svgTextBlockAligned(safeMessage, 150, 490, 46, 4, 26, "#475569", 400)}${campaignBlock}${svgTextBlockAligned("Aponte a câmera do celular para acessar", 150, 790, 48, 1, 22, "#475569", 400)}<rect x="970" y="125" width="520" height="600" rx="32" fill="#ffffff" stroke="${secondaryColor}" stroke-width="6"/><image href="${encodeSvgDataUrl(qrSource)}" x="1015" y="170" width="430" height="430" preserveAspectRatio="xMidYMid meet"/>${svgTextBlock("Aponte a câmera para acessar", 1230, 665, 32, 1, 24, primaryColor, 700)}</svg>`;
}

function buildQrPortraitSvg({ qrSource, churchName, title, message, campaign, primaryColor, secondaryColor, logoDataUrl }: { qrSource: string; churchName: string; title: string; message: string; campaign: string; primaryColor: string; secondaryColor: string; logoDataUrl?: string | null }) {
  const safeChurchName = churchName.trim() || "Sua igreja";
  const safeTitle = title.trim() || "Faça seu cadastro";
  const safeMessage = message.trim() || "Aponte a câmera do celular para acessar.";
  const safeCampaign = campaign.trim();
...
export default function Configuracoes() {
  ...
  const handleDownloadQr = async (format: "landscape-png" | "portrait-png" | "simple-png" | "landscape-svg" | "portrait-svg" = "landscape-png") => {
    ...
    try {
      if (format === "simple-png") {
        ...
        toast.success("PNG simples baixado para uso no celular ou WhatsApp.");
        return;
      }

      const logoDataUrl = await loadImageAsDataUrl(qrLogoUrl);
      const isLandscape = format === "landscape-png" || format === "landscape-svg";
      const artwork = isLandscape ? buildQrLandscapeSvg({
        qrSource,
      ...
      if (format === "landscape-svg" || format === "portrait-svg") {
        const suffix = isLandscape ? "16x9" : "vertical";
        triggerDownload(new Blob([artwork], { type: "image/svg+xml;charset=utf-8" }), `${baseName}-${suffix}.svg`);
        toast.success(`SVG ${isLandscape ? "16:9" : "vertical"} baixado para impressão profissional.`);
        return;
      }

      const width = isLandscape ? 1600 : 1200;
      const height = isLandscape ? 900 : 1500;
      const suffix = isLandscape ? "16x9" : "personalizado";
      const pngBlob = await rasterizeSvg(artwork, width, height);
      triggerDownload(pngBlob, `${baseName}-${suffix}.png`);
      toast.success(`PNG ${isLandscape ? "16:9 para telão" : "vertical"} baixado com a identidade da igreja.`);
    } catch {
          ...
          <TabsContent value="geral">
            <div className="space-y-5">
              ...
              <section className="card-sacred p-5 sm:p-6">
                ...
                <div className="mt-4 grid gap-5 rounded-2xl border border-navy/10 bg-slate-50/70 p-4 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center"><div className="min-w-0"><div className="flex items-center gap-2"><QrCode className="h-4 w-4 text-gold" /><h4 className="font-semibold text-navy">QR Code de cadastro público</h4></div><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Mostre no telão, coloque em convites ou use em um evento. A pessoa preencherá o cadastro sem login; o envio ficará pendente para a equipe da igreja.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><div><Label htmlFor="public-registration-qr-source">Origem do QR Code</Label><select id="public-registration-qr-source" value={qrSource} onChange={(event) => setQrSource(event.target.value as typeof qrSource)} className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground shadow-sm outline-none focus:ring-2 focus:ring-gold/50"><option value="qrcode">QR Code da igreja</option><option value="convite">Convite</option><option value="evento">Evento</option><option value="link">Link compartilhado</option></select></div><div><Label htmlFor="public-registration-qr-campaign">Identificação opcional</Label><Input id="public-registration-qr-campaign" value={qrCampaign} maxLength={120} onChange={(event) => setQrCampaign(event.target.value)} className="mt-1" placeholder="Ex.: Culto de domingo" /></div></div><p className="mt-3 break-all rounded-lg bg-white px-3 py-2 font-mono text-[11px] leading-relaxed text-navy">{registrationQrLink || "Salve o subdomínio para gerar o QR Code"}</p><div className="mt-3 flex flex-col gap-2 sm:flex-row sm:flex-wrap"><Button type="button" className="gap-2 bg-navy text-white hover:bg-navy-light" disabled={!registrationQrLink} onClick={() => void handleDownloadQr("landscape-png")}><QrCode className="h-4 w-4" />Baixar PNG 16:9 para telão</Button><Button type="button" variant="outline" className="gap-2 bg-white text-navy" disabled={!registrationQrLink} onClick={() => void handleDownloadQr("portrait-png")}><QrCode className="h-4 w-4" />PNG vertical</Button><Button type="button" variant="outline" className="gap-2 bg-white text-navy" disabled={!registrationQrLink} onClick={() => void handleDownloadQr("simple-png")}><QrCode className="h-4 w-4" />PNG simples</Button><Button type="button" variant="outline" className="gap-2 bg-white text-navy" disabled={!registrationQrLink} onClick={() => void handleDownloadQr("landscape-svg")}><QrCode className="h-4 w-4" />SVG 16:9</Button><Button type="button" variant="outline" className="gap-2 bg-white text-navy" disabled={!registrationQrLink} onClick={() => void handleDownloadQr("portrait-svg")}><QrCode className="h-4 w-4" />SVG vertical</Button><Button type="button" variant="outline" className="gap-2 bg-white text-navy" disabled={!registrationQrLink} onClick={() => void handleCopyQrLink()}><Copy className="h-4 w-4" />Copiar link</Button></div></div><div className="flex flex-col items-center gap-2 rounded-xl border border-white bg-white p-3 shadow-sm"><div className="w-full max-w-[360px] overflow-hidden rounded-xl border bg-white shadow-sm" style={{ borderColor: `${qrSecondaryColor}66` }}><div className="aspect-video w-full" style={{ backgroundColor: "#ffffff" }}><div className="grid h-full grid-cols-[1.2fr_0.8fr]"><div className="flex min-w-0 flex-col justify-center px-3 py-2 text-left"><div className="flex min-w-0 items-center gap-1.5">{qrLogoUrl ? <img src={qrLogoUrl} alt="Logo da igreja" className="h-6 w-6 shrink-0 rounded-full border object-contain" style={{ borderColor: `${qrSecondaryColor}99` }} /> : <QrCode className="h-5 w-5 shrink-0" style={{ color: qrPrimaryColor }} />}<p className="truncate text-[10px] font-bold" style={{ color: qrPrimaryColor }}>{churchForm.name || "Sua igreja"}</p></div><p className="mt-2 line-clamp-2 text-[11px] font-bold leading-tight text-slate-800">{churchForm.publicRegistrationTitle || "Faça seu cadastro"}</p><p className="mt-1 line-clamp-4 text-[9px] leading-tight text-slate-500">{churchForm.publicRegistrationMessage || "Aponte a câmera do celular para acessar."}</p>{qrCampaign.trim() && <p className="mt-2 line-clamp-1 text-[9px] font-bold" style={{ color: qrPrimaryColor }}>{qrCampaign.trim()}</p>}<p className="mt-auto pt-2 text-[8px] leading-tight text-slate-500">Aponte a câmera para acessar</p></div><div className="flex flex-col items-center justify-center border-l px-2" style={{ borderColor: `${qrSecondaryColor}99`, backgroundColor: `${qrPrimaryColor}08` }}><div className="rounded-md border bg-white p-1" style={{ borderColor: `${qrSecondaryColor}99` }}><QRCodeSVG ref={qrSvgRef} value={registrationQrLink || "https://idefazei.com.br"} size={112} level="M" fgColor={qrPrimaryColor} bgColor="#ffffff" title="QR Code de cadastro público" /></div><p className="mt-1 text-center text-[8px] font-bold leading-tight" style={{ color: qrPrimaryColor }}>Escaneie aqui</p></div></div></div></div><p className="text-center text-[11px] leading-relaxed text-muted-foreground">Prévia 16:9 para o telão: identidade e mensagem à esquerda, QR Code à direita.</p></div></div><Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Compartilhar cadastro</DialogTitle><DialogDescription>Escolha como deseja enviar o convite da {churchForm.name || "igreja"}. A mensagem já incluirá o link oficial.</DialogDescription></DialogHeader><div className="grid gap-2 py-2"><Button type="button" className="h-auto justify-start gap-3 whitespace-normal bg-navy px-4 py-3 text-left text-white hover:bg-navy-light" onClick={() => void handleNativeShare()} disabled={!canUseNativeShare}><Smartphone className="h-5 w-5 shrink-0" /><span><strong className="block">Compartilhar pelo celular</strong><small className="font-normal opacity-80">{canUseNativeShare ? "Escolha WhatsApp, Mensagens, Mail ou outro aplicativo." : "Indisponível neste navegador; use WhatsApp ou Copiar link."}</small></span></Button><Button type="button" variant="outline" className="h-auto justify-start gap-3 whitespace-normal px-4 py-3 text-left" onClick={handleWhatsAppShare}><MessageCircle className="h-5 w-5 shrink-0 text-emerald-600" /><span><strong className="block">WhatsApp</strong><small className="font-normal text-muted-foreground">Abrir com a mensagem e o link já preparados.</small></span></Button><Button type="button" variant="outline" className="h-auto justify-start gap-3 px-4 py-3 text-left" onClick={() => void handleCopyRegistrationLink(true)}><Copy className="h-5 w-5 shrink-0" /><span><strong className="block">Copiar link</strong><small className="font-normal text-muted-foreground">Copiar somente o endereço do cadastro.</small></span></Button></div></DialogContent></Dialog>
              </section>
