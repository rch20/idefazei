...
function buildQrLandscapeSvg({ qrSource, churchName, title, message, campaign, primaryColor, secondaryColor, logoDataUrl }: { qrSource: string; churchName: string; title: string; message: string; campaign: string; primaryColor: string; secondaryColor: string; logoDataUrl?: string | null }) {
  ...
  const logo = logoDataUrl ? `<image href="${logoDataUrl}" x="120" y="100" width="130" height="130" preserveAspectRatio="xMidYMid meet" />` : "";
  const campaignBlock = safeCampaign ? svgTextBlockAligned(safeCampaign, 120, 680, 38, 1, 26, primaryColor, 700) : "";
  return `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="1600" height="900" viewBox="0 0 1600 900"><rect width="1600" height="900" rx="48" fill="#ffffff"/><rect width="1600" height="16" fill="${primaryColor}"/><rect x="42" y="42" width="1516" height="816" rx="34" fill="#ffffff" stroke="${secondaryColor}" stroke-width="5"/><rect x="760" y="80" width="2" height="740" fill="${secondaryColor}" opacity="0.45"/><circle cx="1160" cy="430" r="330" fill="${primaryColor}" opacity="0.06"/>${logo}${svgTextBlockAligned(safeChurchName, 120, 290, 34, 2, 40, primaryColor, 700)}${svgTextBlockAligned(safeTitle, 120, 385, 36, 2, 32, "#172033", 700)}${svgTextBlockAligned(safeMessage, 120, 480, 42, 4, 24, "#475569", 400)}${campaignBlock}${svgTextBlockAligned("Leia com a câmera", 120, 790, 36, 1, 20, "#475569", 400)}<rect x="800" y="65" width="735" height="770" rx="36" fill="#ffffff" stroke="${secondaryColor}" stroke-width="6"/><image href="${encodeSvgDataUrl(qrSource)}" x="855" y="105" width="625" height="625" preserveAspectRatio="xMidYMid meet"/>${svgTextBlock("Escaneie o QR Code", 1167, 775, 32, 1, 22, primaryColor, 700)}</svg>`;
}
