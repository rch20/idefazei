...
import { formatDatePtBr, formatEventTimeRange } from "@/lib/treasury";
...
export default function Inicio() {
  ...
  const announcement = data?.publicAnnouncements?.[0];
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {event ? (
                <article className="card-sacred p-4">
                  <div className="flex items-start gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-navy/5 text-navy"><CalendarDays className="h-5 w-5" aria-hidden="true" /></div>
                    <div className="min-w-0">
                      <p className="text-xs font-semibold uppercase tracking-[0.12em] text-gold">Próximo evento</p>
                      <h3 className="mt-1 truncate font-semibold text-navy">{event.name}</h3>
                      <p className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground"><CalendarDays className="h-3.5 w-3.5" aria-hidden="true" />{formatDatePtBr(event.startDate)}</p>
                      {event.startTime && <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><Clock3 className="h-3.5 w-3.5" aria-hidden="true" />{formatEventTimeRange(event.startTime, event.endTime)}</p>}
                      {event.location && <p className="mt-1 flex items-center gap-1.5 truncate text-xs text-muted-foreground"><MapPin className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />{event.location}</p>}
                    </div>
                  </div>
                </article>
              ) : (
              ...
              {announcement ? (
                ...
                <article className="card-sacred p-4">
                  <div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gold/10 text-gold"><Bell className="h-5 w-5" aria-hidden="true" /></div><div><h3 className="font-semibold text-navy">Nenhum aviso novo</h3><p className="mt-1 text-sm text-muted-foreground">Os comunicados da igreja aparecerão aqui quando forem publicados.</p></div></div>
                </article>
              )}
            </div>
          </section>
