...
export default function EncontroComDeusDetalhe() {
  ...
  const [servantError, setServantError] = useState<string | null>(null);
  ...
  const reviewForm = trpc.encontro.reviewDiscipleForm.useMutation({ onSuccess: () => { toast.success("Ficha revisada."); setDiscipleDetails(null); setReviewError(null); invalidateEncounter(); }, onError: (error) => { setReviewError(error.message); toast.error(error.message); } });
  ...
  const assignServant = trpc.encontro.assignServant.useMutation({ onSuccess: () => { toast.success("Servo atribuído."); setServantOpen(false); setServantError(null); setServantForm(emptyServantForm); invalidateEncounter(); }, onError: (error) => { setServantError(error.message); toast.error(error.message); } });
  ...
  const createChecklist = trpc.encontro.createChecklistItem.useMutation({ onSuccess: () => { toast.success("Pendência adicionada."); setChecklistOpen(false); setChecklistError(null); setChecklistForm(emptyChecklistForm); invalidateEncounter(); }, onError: (error) => { setChecklistError(error.message); toast.error(error.message); } });
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <Tabs defaultValue="visao" className="space-y-4"><TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-xl bg-[#1e3a5f]/5 p-1"><TabsTrigger value="visao">Visão geral</TabsTrigger><TabsTrigger value="discipulos">Discípulos</TabsTrigger><TabsTrigger value="servos">Servos e equipes</TabsTrigger><TabsTrigger value="checklist">Checklist</TabsTrigger><TabsTrigger value="configuracoes"><Settings className="mr-1.5 h-4 w-4" /> Configurações</TabsTrigger><TabsTrigger value="historico">Histórico</TabsTrigger></TabsList>
        ...
        <TabsContent value="servos" className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-xl font-bold text-[#1e3a5f]">Servos e equipes</h2><p className="text-sm text-muted-foreground">Funções específicas deste encontro, sem alterar o cargo permanente da Pessoa.</p></div><div className="flex gap-2"><Dialog open={teamOpen} onOpenChange={setTeamOpen}><DialogTrigger asChild><Button variant="outline"><Plus className="mr-2 h-4 w-4" /> Nova equipe</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Adicionar equipe ou frente</DialogTitle></DialogHeader><div className="space-y-4"><div><Label>Nome *</Label><Input value={teamForm.name} onChange={(event) => setTeamForm({ ...teamForm, name: event.target.value })} /></div><div><Label>Categoria</Label><Select value={teamForm.category} onValueChange={(value) => setTeamForm({ ...teamForm, category: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(TEAM_CATEGORY_LABELS).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div><div><Label>Quantidade necessária</Label><Input type="number" min="1" value={teamForm.requiredCount} onChange={(event) => setTeamForm({ ...teamForm, requiredCount: event.target.value })} /></div><div><Label>Observações</Label><Textarea value={teamForm.notes} onChange={(event) => setTeamForm({ ...teamForm, notes: event.target.value })} /></div><Button className="w-full" disabled={!teamForm.name.trim()} onClick={() => createTeam.mutate({ churchId, eventId, name: teamForm.name, category: teamForm.category as any, requiredCount: teamForm.requiredCount ? Number(teamForm.requiredCount) : null, notes: teamForm.notes || null })}>Adicionar equipe</Button></div></DialogContent></Dialog><Dialog open={servantOpen} onOpenChange={(nextOpen) => { setServantOpen(nextOpen); if (nextOpen) setServantError(null); }}><DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" /> Adicionar servo</Button></DialogTrigger><AdaptiveFormDialogContent><div className={adaptiveFormDialogHeaderClassName}><DialogTitle>Atribuir servo</DialogTitle><DialogDescription>Escolha uma função do catálogo ou informe uma função manual válida somente para este encontro.</DialogDescription></div><AdaptiveFormDialogBody className="space-y-4">{servantError && <div role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{servantError}</div>}<div><Label>Pessoa *</Label><Select value={servantForm.personId} onValueChange={(value) => { setServantError(null); setServantForm({ ...servantForm, personId: value }); }}><SelectTrigger><SelectValue placeholder="Selecione a Pessoa" /></SelectTrigger><SelectContent>{(people.data ?? []).map((person) => <SelectItem key={person.id} value={String(person.id)}>{person.fullName}</SelectItem>)}</SelectContent></Select></div><div><Label>Equipe</Label><Select value={servantForm.teamId || "sem_equipe"} onValueChange={(value) => { setServantError(null); setServantForm({ ...servantForm, teamId: value === "sem_equipe" ? "" : value, roleName: value === "sem_equipe" || servantForm.roleMode === "manual" ? servantForm.roleName : teams.data?.find((team) => String(team.id) === value)?.name ?? "" }); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="sem_equipe">Sem equipe</SelectItem>{(teams.data ?? []).map((team) => <SelectItem key={team.id} value={String(team.id)}>{team.name}</SelectItem>)}</SelectContent></Select></div><div><Label>Origem da função</Label><Select value={servantForm.roleMode} onValueChange={(value) => { setServantError(null); setServantForm({ ...servantForm, roleMode: value, roleName: value === "manual" ? "" : teams.data?.find((team) => String(team.id) === servantForm.teamId)?.name ?? "" }); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="catalogo">Função/equipe predefinida</SelectItem><SelectItem value="manual">Adicionar função manual</SelectItem></SelectContent></Select></div><div><Label>{servantForm.roleMode === "manual" ? "Nome da função manual *" : "Função *"}</Label><Input value={servantForm.roleName} onChange={(event) => { setServantError(null); setServantForm({ ...servantForm, roleName: event.target.value }); }} placeholder={servantForm.roleMode === "manual" ? "Ex.: Apoio aos quartos" : "Selecione uma equipe ou informe a função"} /></div><div><Label>Tipo de atribuição</Label><Select value={servantForm.assignmentType} onValueChange={(value) => setServantForm({ ...servantForm, assignmentType: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="responsavel">Responsável</SelectItem><SelectItem value="membro">Membro</SelectItem><SelectItem value="substituto">Substituto</SelectItem></SelectContent></Select></div><div><Label>Observações</Label><Textarea value={servantForm.notes} onChange={(event) => { setServantError(null); setServantForm({ ...servantForm, notes: event.target.value }); }} /></div></AdaptiveFormDialogBody><AdaptiveFormDialogFooter><Button type="button" variant="outline" onClick={() => setServantOpen(false)} disabled={assignServant.isPending}>Cancelar</Button><Button disabled={!servantForm.personId || !servantForm.roleName.trim() || assignServant.isPending} onClick={() => assignServant.mutate({ churchId, eventId, personId: Number(servantForm.personId), teamId: servantForm.teamId ? Number(servantForm.teamId) : null, roleKey: servantForm.roleMode === "manual" ? null : servantForm.roleName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "_"), roleName: servantForm.roleName, roleSource: servantForm.roleMode as any, assignmentType: servantForm.assignmentType as any, notes: servantForm.notes || null })}>{assignServant.isPending ? "Salvando..." : "Confirmar atribuição"}</Button></AdaptiveFormDialogFooter></AdaptiveFormDialogContent></Dialog></div></div>
        ...
        <TabsContent value="checklist" className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-display text-xl font-bold text-[#1e3a5f]">Checklist de preparação</h2><p className="text-sm text-muted-foreground">Acompanhe prazos, responsáveis e pendências antes do encontro.</p></div><Dialog open={checklistOpen} onOpenChange={(nextOpen) => { setChecklistOpen(nextOpen); if (nextOpen) setChecklistError(null); }}><DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" /> Nova pendência</Button></DialogTrigger><AdaptiveFormDialogContent><div className={adaptiveFormDialogHeaderClassName}><DialogTitle>Adicionar pendência</DialogTitle><p className="mt-1 text-sm text-muted-foreground">Registre a tarefa, o responsável e o prazo para a preparação do encontro.</p></div><AdaptiveFormDialogBody className="space-y-4">{checklistError && <div role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{checklistError}</div>}<div><Label>Tarefa *</Label><Input value={checklistForm.title} onChange={(event) => { setChecklistError(null); setChecklistForm({ ...checklistForm, title: event.target.value }); }} /></div><div><Label>Categoria</Label><Select value={checklistForm.category} onValueChange={(value) => { setChecklistError(null); setChecklistForm({ ...checklistForm, category: value }); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(CHECKLIST_CATEGORY_LABELS).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div><div><Label>Responsável</Label><Select value={checklistForm.assignedPersonId || "sem_responsavel"} onValueChange={(value) => { setChecklistError(null); setChecklistForm({ ...checklistForm, assignedPersonId: value === "sem_responsavel" ? "" : value }); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="sem_responsavel">Definir depois</SelectItem>{(people.data ?? []).map((person) => <SelectItem key={person.id} value={String(person.id)}>{person.fullName}</SelectItem>)}</SelectContent></Select></div><div><Label>Prazo</Label><Input type="datetime-local" value={checklistForm.dueAt} onChange={(event) => { setChecklistError(null); setChecklistForm({ ...checklistForm, dueAt: event.target.value }); }} /></div><div><Label>Observações</Label><Textarea value={checklistForm.notes} onChange={(event) => { setChecklistError(null); setChecklistForm({ ...checklistForm, notes: event.target.value }); }} /></div></AdaptiveFormDialogBody><AdaptiveFormDialogFooter><Button type="button" variant="outline" onClick={() => setChecklistOpen(false)} disabled={createChecklist.isPending}>Cancelar</Button><Button disabled={!checklistForm.title.trim() || createChecklist.isPending} onClick={() => createChecklist.mutate({ churchId, eventId, title: checklistForm.title, category: checklistForm.category as any, assignedPersonId: checklistForm.assignedPersonId ? Number(checklistForm.assignedPersonId) : null, dueAt: checklistForm.dueAt ? new Date(checklistForm.dueAt) : null, notes: checklistForm.notes || null })}>{createChecklist.isPending ? "Salvando..." : "Adicionar pendência"}</Button></AdaptiveFormDialogFooter></AdaptiveFormDialogContent></Dialog></div><Card><CardContent className="p-0"><div className="divide-y">{(checklist.data ?? []).length === 0 ? <Empty text="Nenhuma pendência cadastrada." /> : (checklist.data ?? []).map(({ item, assignee }) => <div key={item.id} className="grid gap-3 p-4 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="font-semibold text-[#1e3a5f]">{item.title}</p><p className="mt-1 text-xs text-muted-foreground">{CHECKLIST_CATEGORY_LABELS[item.category]}{assignee?.fullName ? ` · ${assignee.fullName}` : ""}{item.dueAt ? ` · até ${formatDateTime(item.dueAt)}` : ""}</p></div><Select value={item.status} onValueChange={(status) => updateChecklist.mutate({ id: item.id, eventId, churchId, status: status as any })}><SelectTrigger className="w-full sm:w-44"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(CHECKLIST_STATUS_LABELS).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>)}</div></CardContent></Card></TabsContent>
      ...
      <Dialog open={Boolean(discipleDetails)} onOpenChange={(open) => { if (!open) { setDiscipleDetails(null); setReviewError(null); } }}>
        <AdaptiveFormDialogContent className="sm:max-w-2xl">
          <div className={adaptiveFormDialogHeaderClassName}>
            <DialogTitle className="font-display text-[#1e3a5f]">Ficha do discípulo</DialogTitle>
            <DialogDescription>Dados privados para organização e acompanhamento deste encontro.</DialogDescription>
          </div>
          <AdaptiveFormDialogBody className="space-y-5">
            {discipleDetails?.form && <>
              {reviewError && <div role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800">{reviewError}</div>}
              <section className="grid gap-3 rounded-2xl bg-[#1e3a5f]/5 p-4 sm:grid-cols-2"><Info label="Nome completo" value={discipleDetails.form.fullName} /><Info label="Idade" value={`${discipleDetails.form.age} anos`} /><Info label="Telefone" value={discipleDetails.form.phone} /><Info label="Igreja que participa" value={discipleDetails.form.attendingChurch} /></section>
              <section className="grid gap-3 sm:grid-cols-2"><Info label="Mãe, pai ou responsável" value={discipleDetails.form.guardianName} /><Info label="Telefone do responsável" value={discipleDetails.form.guardianPhone} /><Info label="Amigo de confiança" value={discipleDetails.form.friendName} /><Info label="Telefone do amigo" value={discipleDetails.form.friendPhone} /><Info label="Quem convidou" value={discipleDetails.form.invitedByName} /><Info label="Ficha enviada em" value={formatDateTime(discipleDetails.form.submittedAt)} /></section>
              <div><Label>Situação da ficha</Label><Select value={reviewStatus} onValueChange={(value) => { setReviewError(null); setReviewStatus(value); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Object.entries(REVIEW_STATUS_LABELS).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
              <div><Label>Observações da análise</Label><Textarea rows={3} value={reviewNotes} onChange={(event) => { setReviewError(null); setReviewNotes(event.target.value); }} placeholder="Orientações internas da coordenação" /></div>
            </>}
          </AdaptiveFormDialogBody>
          <AdaptiveFormDialogFooter>
            <Button type="button" variant="outline" onClick={() => { setDiscipleDetails(null); setReviewError(null); }} disabled={reviewForm.isPending}>Fechar</Button>
            <Button disabled={!discipleDetails?.form || reviewForm.isPending} onClick={() => discipleDetails?.form && reviewForm.mutate({ id: discipleDetails.form.id, eventId, churchId, reviewStatus: reviewStatus as any, reviewNotes: reviewNotes.trim() || null })}>{reviewForm.isPending ? "Salvando..." : "Salvar revisão da ficha"}</Button>
          </AdaptiveFormDialogFooter>
