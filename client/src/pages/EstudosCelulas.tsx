...
import { BookOpen, Download, ExternalLink, FileDown, FileText, FileUp, Link2, Loader2, Pencil, Plus, ShieldCheck, Trash2, Upload, X } from "lucide-react";
...
function AttachmentList({ studyId, attachments, canManage, churchId, onChanged, compact = false }: { studyId: number; attachments: Attachment[]; canManage: boolean; churchId: number; onChanged: () => void; compact?: boolean }) {
  ...
  return <div className={compact ? "mt-3" : "mt-4 rounded-xl border border-[#c9a84c]/20 bg-[#fdfaf1] p-3"}>
    {!compact ? <div className="mb-2 flex items-center justify-between gap-2"><p className="flex items-center gap-2 text-sm font-semibold text-[#1e3a5f]"><FileText className="h-4 w-4 text-[#c9a84c]" />Materiais do estudo</p>{canManage ? <div className="flex flex-wrap gap-2"><input ref={fileRef} type="file" className="hidden" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt,.png,.jpg,.jpeg,.webp,.mp3,.m4a,.wav,.ogg" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadSelectedFile(file); event.target.value = ""; }} /><Button type="button" size="sm" variant="outline" disabled={uploading} onClick={() => fileRef.current?.click()}>{uploading ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" /> : <Upload className="mr-1 h-3.5 w-3.5" />}Arquivo</Button><Button type="button" size="sm" variant="outline" onClick={() => setShowLink((value) => !value)}><Link2 className="mr-1 h-3.5 w-3.5" />Link externo</Button></div> : null}</div> : null}
    {targetFile && uploading ? <p className="mb-2 text-xs text-muted-foreground">Enviando {targetFile.name}…</p> : null}
    {!compact && showLink ? <div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)_auto]"><Input value={linkTitle} onChange={(event) => setLinkTitle(event.target.value)} placeholder="Nome do material" /><Input value={linkUrl} onChange={(event) => setLinkUrl(event.target.value)} placeholder="https://…" inputMode="url" /><Button type="button" disabled={linkTitle.trim().length < 2 || !linkUrl.startsWith("https://") || attachLink.isPending} onClick={() => attachLink.mutate({ churchId, studyId, title: linkTitle, url: linkUrl })}>Adicionar</Button></div> : null}
...
function LeaderStudyCards({ rows, churchId, onRead, onChanged }: { rows: StudyWithAttachments[]; churchId: number; onRead: (studyId: number) => void; onChanged: () => void }) {
  return <div className="space-y-4">{rows.map(({ study, attachments }) => {
    const preview = study.objective || study.introduction || study.practicalApplication || "Estudo oficial preparado para aplicação na Célula.";
    return <Card key={study.id} className="overflow-hidden border-[#c9a84c]/25 shadow-sm">
      <CardHeader className="space-y-3 pb-3">
        <div className="flex flex-wrap items-center gap-2">
          <Badge>Publicado</Badge>
          <span className="text-xs font-medium uppercase tracking-wide text-[#c9a84c]">Semana de {formatWeek(study.weekStart)}</span>
        </div>
        <div>
          <CardTitle className="text-xl text-[#1e3a5f]">{study.title}</CardTitle>
          {study.biblicalText ? <p className="mt-2 text-sm font-medium text-muted-foreground">{study.biblicalText}</p> : null}
        </div>
      </CardHeader>
      <CardContent className="space-y-4 pt-0">
        <p className="line-clamp-3 text-sm leading-6 text-muted-foreground">{preview}</p>
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <Button type="button" className="w-full bg-[#1e3a5f] text-white sm:w-auto" onClick={() => onRead(study.id)}><BookOpen className="mr-2 h-4 w-4" />Ver estudo</Button>
        </div>
        <AttachmentList studyId={study.id} attachments={attachments} canManage={false} churchId={churchId} onChanged={onChanged} compact />
      </CardContent>
    </Card>;
  })}</div>;
}

function StudyReaderDialog({ row, open, onOpenChange, churchId, onChanged }: { row: StudyWithAttachments | null; open: boolean; onOpenChange: (open: boolean) => void; churchId: number; onChanged: () => void }) {
  if (!row) return null;
  const { study, attachments } = row;
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="z-[200] max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-3xl overflow-y-auto p-4 sm:max-w-3xl sm:p-6">
      <DialogHeader className="text-left">
        <div className="flex flex-wrap items-center gap-2 pr-8"><Badge>Publicado</Badge><span className="text-xs font-medium uppercase tracking-wide text-[#c9a84c]">Semana de {formatWeek(study.weekStart)}</span></div>
        <DialogTitle className="text-2xl text-[#1e3a5f]">{study.title}</DialogTitle>
        <DialogDescription>{study.biblicalText || "Roteiro oficial para aplicação na Célula."}</DialogDescription>
      </DialogHeader>
      <div className="space-y-5">
        <section><h3 className="mb-1 text-sm font-semibold text-[#1e3a5f]">Objetivo</h3><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{study.objective || "Não informado."}</p></section>
        <section><h3 className="mb-1 text-sm font-semibold text-[#1e3a5f]">Introdução</h3><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{study.introduction || "Não informado."}</p></section>
        <section><h3 className="mb-1 text-sm font-semibold text-[#1e3a5f]">Desenvolvimento</h3><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{study.development || "Não informado."}</p></section>
        <section><h3 className="mb-1 text-sm font-semibold text-[#1e3a5f]">Perguntas para discussão</h3><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{study.discussionQuestions || "Não informado."}</p></section>
        <section><h3 className="mb-1 text-sm font-semibold text-[#1e3a5f]">Aplicação prática</h3><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{study.practicalApplication || "Não informado."}</p></section>
        <section><h3 className="mb-1 text-sm font-semibold text-[#1e3a5f]">Oração</h3><p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{study.prayer || "Não informado."}</p></section>
        <AttachmentList studyId={study.id} attachments={attachments} canManage={false} churchId={churchId} onChanged={onChanged} />
      </div>
    </DialogContent>
  </Dialog>;
}
...
export default function EstudosCelulas() {
  ...
  const [readerStudyId, setReaderStudyId] = useState<number | null>(null);
  ...
  const readerRow = rows.find(({ study }) => study.id === readerStudyId) ?? null;
