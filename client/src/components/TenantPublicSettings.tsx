...
export function TenantPublicSettings() {
  ...
  return <div className="space-y-6">
    ...
    <div className="mb-1 flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">Edite à esquerda e acompanhe a prévia protegida à direita.</p><Button type="button" variant="outline" size="sm" className="shrink-0 xl:hidden" onClick={() => document.getElementById("tenant-public-preview")?.scrollIntoView({ behavior: "smooth", block: "start" })}><Eye className="mr-1.5 h-4 w-4" />Ver prévia</Button></div>
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,0.8fr)]">
      <div className="min-w-0 space-y-5">
        ...
        <div className="sticky bottom-3 z-10 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-background/95 p-3 shadow-lg backdrop-blur sm:flex-row sm:items-center sm:justify-end"><span className="mr-auto hidden text-xs text-muted-foreground sm:block">Rascunho só fica público depois de publicar.</span><Button type="button" variant="outline" disabled={saveDraft.isPending || publish.isPending} onClick={save}><Save className="mr-2 h-4 w-4" />{saveDraft.isPending ? "Salvando..." : "Salvar rascunho"}</Button><Button type="button" className="bg-navy text-white hover:bg-navy-light" disabled={saveDraft.isPending || publish.isPending} onClick={publishPage}><Send className="mr-2 h-4 w-4" />{publish.isPending ? "Publicando..." : "Publicar página"}</Button></div>
      </div>
